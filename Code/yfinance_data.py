...
plt.title('Distribution of Securities by Type', fontsize=30, weight='bold')
plt.xlabel('Type of Security', fontsize=26, weight='bold')
plt.ylabel('Number of Securities', fontsize=26, weight='bold')
plt.xticks(rotation=0, fontsize=22)
plt.yticks(fontsize=22)
plt.grid(True, linestyle='--', alpha=0.7)
for p in ax.patches:
    ax.annotate(f'{p.get_height()}', (p.get_x() + p.get_width() / 2., p.get_height()), ha='center', va='baseline', color='black', fontsize=24, weight='bold')
plt.tight_layout()
...
for column in cumulative_returns_by_type.columns:
    plt.plot(cumulative_returns_by_type.index, cumulative_returns_by_type[column] * 100 - 100, label=column, linewidth=3, color=colorblind_palette.pop(0))
plt.title('Cumulative Returns Over Time by Security Type', fontsize=30, weight='bold')
plt.xlabel('Date', fontsize=26, weight='bold')
plt.ylabel('Cumulative Return (%)', fontsize=26, weight='bold')
plt.legend(title='Security Type', fontsize=22, title_fontsize=24, loc='upper left')
